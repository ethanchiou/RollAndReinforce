...
- **2026-05-19** — **MVP zombie roster complete** (5/5). Each type a separate procedural `.blend` + `.fbx` under `Assets/zombies/`, distinct silhouette per SPEC § Visual direction. All under the 1500-tri budget.
  ...
  |---------|-----------------------|------|---------------------------|----------|
  | Walker  | `zombie-base`         | 528  | Hunched, outstretched     | Yellow   |
  | Runner  | `zombie-runner`       | 408  | Forward-leaning sprinter  | Red      |
  | Brute   | `zombie-brute`        | 600  | Wide, no neck, massive arms | Red (asym) |
  | Climber | `zombie-climber`      | 516  | Tall mantis with claws    | Purple   |
  | Spitter | `zombie-spitter`      | 456  | Back-tilted, acid sac     | Acid green |
- **2026-05-19** — First art asset: **base zombie (Walker)** shipped via procedural `bpy` script through the Blender MCP bridge. 352 verts / 528 tris, ~5-stud height, Roblox-friendly axes. Style matches SPEC § Visual direction — blocky-cartoony, asymmetric glowing pinprick eyes (bright left / dim right), open jagged-tooth mouth, layered bandages with bloodstain on the head wrap, torn shirt with rot peek-through, procedural noise mottling on skin + cloth, subtle forward hunch. Arms ~2.0 forward reach with a slight elbow bend (revised from initial 2.5 zigzag).
